...
현재 버전: **v60** (`BUILD="2026-08-24 v60"`, `BUILD_NO=60`, `CACHE="kel-v60"`). v35 배포 후 v36에서 포켓몬 무대·트레이너 카드·관전 모드 추가.
...
- 결과창 확인 기록은 kel_bt_seen **목록**(최근 80개) — 단일 값이던 시절 done 배틀 2개가 번갈아 재등장하던 좀비 결과창 버그 v47에서 수리. **v60:** 결과창 도장을 닫을 때가 아니라 **자동으로 뜨는 순간** 즉시(패치 재시작으로 부활하던 잔존 케이스 종결, 자동 팝 창은 30분 이내로 축소). 난투: done도 1회 결과 표시(myBrawl 포함+즉시 도장)+보드에 1시간 결과 노출, invite에 🗑해산(개설자)/🚪나가기(참가자), play에 🏳️기권(__ff→hp0 탈락). 보상 지급 가드: `kel_bt_paid_<id>` (각자 자기 것만 갱신).
