...
function kelLoad35() {
  clearTimeout(__k35t);
  __k35t = setTimeout(kelLoad35Now, 350);
}
async function kelLoad35Now() {
  if (!sb) return;
  try {
...
function openBattleGuide() {
  ...
  box.innerHTML = '<div class="armory"><button class="ovx" data-ovx>✕</button>' +
    ...
    '<p><b>무응답</b> — 내가 픽하고 상대가 45초 넘게 조용하면 자동 패스 처리, 3턴 연속이면 몰수패. 앱을 꺼도 도망은 못 간다.</p>' +
...
function dedupeBattles(arr) {
  var seen = {}, out = [];
  arr.slice().sort(function (x, y) { return y.id - x.id; }).forEach(function (b) {
    var k = [b.a, b.b].sort().join("|") + "|" + (b.phase === "play" ? "p" : "s");
    if (seen[k]) { b.__dupe = true; return; }
    seen[k] = 1; out.push(b);
  });
  return out;
}
...
var __duelBusy = false;
async function duelSend(mid, when) {
  if (__duelBusy) return;
  if (openWith(mid)) return toast(nameOf(mid) + "와는 이미 잡혀 있어");
  __duelBusy = true;
  setTimeout(function () { __duelBusy = false; }, 2500);
...
async function sweepInvites() {
  // 같은 매치업 중복분(연타 사고) → 최신 것만 남기고 자동 취소
  var dupes = [];
  dedupeBattles(store35.battles);
  store35.battles.forEach(function (b) {
    if (b.__dupe && (b.phase === "invite" || b.phase === "sched")) dupes.push(b);
  });
  for (var d = 0; d < dupes.length; d++)
    await sb.from("battles").update({ phase: "canceled" }).eq("id", dupes[d].id).in("phase", ["invite", "sched"]);
  if (dupes.length) { toast("🧹 중복 도전장 " + dupes.length + "건 정리"); }
...
function battleWatch() {
  ...
  if (play && play.pick_a && play.pick_b && play.phase === "play") {
    ...
    var canA = play.a === me;
    var canB = play.b === me && Date.now() - window.__bpAt[rk] > 12000;
    if ((canA || canB) && window.__rtLock !== rk) {
      window.__rtLock = rk;
      resolveTurn(play, canB).finally(function () { if (window.__rtLock === rk) window.__rtLock = null; });
    }
  }
  // 내가 픽했는데 상대가 45초째 무응답 → 상대를 자동 패스 처리 (AFK)
  if (play && play.phase === "play") {
    var mineP = play.a === me ? play.pick_a : play.pick_b;
    var foeP  = play.a === me ? play.pick_b : play.pick_a;
    var wk = "w" + play.id + ":" + play.turn;
    window.__bpAt = window.__bpAt || {};
    if (mineP && !foeP) {
      if (!window.__bpAt[wk]) window.__bpAt[wk] = Date.now();
      else if (Date.now() - window.__bpAt[wk] > 45000) {
        window.__bpAt[wk] = Date.now() + 9e12;   // 1회만
        var col = play.a === me ? "pick_b" : "pick_a", pt = {}; pt[col] = "__afk";
        sb.from("battles").update(pt).eq("id", play.id).eq("phase", "play").eq("turn", play.turn).is(col, null)
          .then(function () { kelLoad35(); });
      }
    } else delete window.__bpAt[wk];
  }
  var g = play || justDone;
  if (g) {
...
var btBnTimer = null, __bnSig = "";
function renderBattleBanners(list) {
  var wrap = document.getElementById("duel-banners");
  if (!list || !list.length) { clearInterval(btBnTimer); __bnSig = ""; if (wrap) wrap.remove(); return; }
  ...
  var sig = list.slice(0, 4).map(function (g) { return g.id + g.phase + ((g.state || {}).early || ""); }).join(",");
  var rebuilt = sig !== __bnSig;
  if (rebuilt) {
    __bnSig = sig;
    var rows = "";
    list.slice(0, 4).forEach(function (g) {
      ...
      if (g.phase === "invite" && g.b === me) {
        rows += '<div class="duel-row" data-bid="' + g.id + '" data-kind="inv">⚔️ <b>' + esc(nameOf(g.a)) + "</b> " + hm + ' 신청 · <b class="bn-t"></b>' +
        ...
        if (early && early !== me) {
          ...
          rows += '<div class="duel-row" data-bid="' + g.id + '" data-kind="sched" data-at="' + new Date(g.sched_at || g.created_at).getTime() + '">' +
            '⚔️ ' + hm + " vs " + esc(nameOf(en)) + ' — <b class="bn-t"></b>' +
            '<span class="duel-acts"><span class="bn-enter"></span>' +
            (early === me ? '<span class="muted2">⚡ 제안됨</span>' : '<button class="btn small ghost" data-act="early">⚡ 지금 하자</button>') +
            '<button class="btn small ghost" data-act="cancel">✖</button></span></div>';
        }
      }
    });
    wrap.innerHTML = rows;
    $$(".duel-row [data-act]", wrap).forEach(function (b2) {
      b2.onclick = async function () {
        var row = b2.closest(".duel-row"), g = list.find(function (x) { return String(x.id) === row.dataset.bid; });
        if (!g) return;
        var en2 = g.a === me ? g.b : g.a;
        ...
        if (act === "no") { await sb.from("battles").update({ phase: "declined" }).eq("id", g.id).eq("phase", "invite"); kelLoad35(); }
          ...
          if (immediate && myPlay()) { toast("지금 싸우는 판부터 끝내 — 예약으로 잡아둘게"); await sb.from("battles").update({ phase: "sched" }).eq("id", g.id).eq("phase", "invite"); return kelLoad35(); }
  ...
  function tick() {
    $$(".duel-row", wrap).forEach(function (row) {
      var g = list.find(function (x) { return String(x.id) === row.dataset.bid; });
      if (!g) return;
      var t = row.querySelector(".bn-t");
      if (row.dataset.kind === "inv" && t) {
      ...
      } else if (row.dataset.kind === "sched") {
        var lf = Number(row.dataset.at) - Date.now();
        if (lf <= 0) {
          if (t) t.textContent = "결투 시간!";
          var slot = row.querySelector(".bn-enter");
          if (slot && !slot.firstChild) {
            var eb = document.createElement("button");
            eb.className = "btn small"; eb.textContent = "입장"; eb.dataset.act = "enter";
            eb.onclick = function () { earlyGo(g); };
            slot.appendChild(eb);
          }
        } else if (t) {
          var mn = Math.floor(lf / 60000), sc = Math.floor(lf % 60000 / 1000);
          t.textContent = mn > 99 ? mn + "분" : mn + ":" + String(sc).padStart(2, "0");
        }
      }
    });
  }
  ...
  btBnTimer = setInterval(tick, 1000);
}
...
function pkStageHtml(g, ns) {
  ns = ns || "bt";
  var st = g.state || {};
  var L = (g.a === me || g.b === me) ? me : g.a;      // 좌하단(큰 쪽): 나, 관전이면 a
  var R = L === g.a ? g.b : g.a;
  var mp = {}; mp[L] = ns + "-mine"; mp[R] = ns + "-foe"; __pkMaps[ns] = mp;
  var cL = charOf(L) || { id: "x", em: "🎩" }, cR = charOf(R) || { id: "x", em: "🎩" };
  return '<div class="pk-stage" id="' + ns + '-stage">' +
...
function playTheatre(g, ns, replay) {
  ns = ns || "bt";
  var st = g.state || {}, logs = st.log || [];
  var seen = __thSeen[ns];
  if (!seen || seen.id !== g.id) {
    __thSeen[ns] = { id: g.id, n: replay ? Math.max(0, logs.length - 8) : logs.length };
    if (!replay) return;
    seen = __thSeen[ns];
  }
  ...
  fresh.forEach(function (l, i) {
    setTimeout(function () {
      var box2 = document.getElementById(ns + "-box");
      if (box2) box2.innerHTML = '<div class="bt-line">' + l.txt + "</div>";
      if (!l.who) { if (l.fx) FX2.cast(l.fx, { quick: true }); return; }
      var atkId = mp[l.who] || (ns + "-foe");
      var defId = atkId === ns + "-mine" ? ns + "-foe" : ns + "-mine";
  ...
  // 막이 끝나고 기절 처리
  setTimeout(function () {
    Object.keys(mp).forEach(function (mm) {
      if ((st.hp || {})[mm] <= 0) {
        var el = document.getElementById(mp[mm]);
...
function openBattle() {
  ...
  if (g.phase === "done") {
    var win = g.winner;
    box.innerHTML = '<div class="armory bt-wrap pk-wrap"><button class="ovx" data-ovx>✕</button>' + pkStageHtml(g, "bt") +
    ...
    setTimeout(function () {
      var mpb = __pkMaps.bt || {};
      var wEl = win ? document.getElementById(mpb[win] || "bt-foe") : null;
      var lEl = wEl ? document.getElementById(wEl.id === "bt-mine" ? "bt-foe" : "bt-mine") : null;
...
async function resolveTurn(g, force) {
  ...
  if (g.a !== me && !force) { btG = g; openBattle(); return; }
  var st = JSON.parse(JSON.stringify(g.state || {}));
  ...
  // 3턴 연속 무응답 → 몰수패
  if (patch.phase !== "done") {
    var afkOut = [g.a, g.b].find(function (mm) { return (st.afk[mm] || 0) >= 3; });
    if (afkOut) {
      var other = afkOut === g.a ? g.b : g.a;
      line("🏳️ " + nameOf(afkOut) + " 3턴 연속 무응답 — 몰수패!");
      patch.phase = "done"; patch.winner = other;
...
function openSpectate(g, opts) {
  opts = opts || {};
  if (g.a === me || g.b === me) { btG = g; __btUi.sig = ""; return openBattle(); }
  specId = g.id;
  ...
  __specCore = g.phase + "|" + g.turn + "|" + ((st.log || []).length);
  box.innerHTML = '<div class="armory bt-wrap pk-wrap"><button class="ovx" data-ovx>✕</button>' +
    pkStageHtml(g, "sp") +
    '<div class="pk-box" id="sp-box">' + lastLog + '</div>' +
    (g.phase === "done"
      ? '<div class="liar-banner">🏁 ' + (g.winner ? esc(nameOf(g.winner)) + " 승리!" : "무승부") + "</div>"
      : '<div class="bt-pickmsg" id="sp-picks">' + specPicksHtml(g) + "</div>") +
    '</div>';
  box.scrollTop = keep;
  playTheatre(g, "sp", opts.replay);
...
function spectateWatch() {
  ...
  var core = g.phase + "|" + g.turn + "|" + ((g.state || {}).log || []).length;
  if (core === __specCore) {
    var pi = document.getElementById("sp-picks");        // 픽 표시만 살짝 갱신 — 화면 재생성 없음
    if (pi) pi.innerHTML = specPicksHtml(g);
    return;
  }
  if (__pkBusy.sp && Date.now() < __pkBusy.sp) {          // 연출 끝날 때까지 리빌드 유예
    clearTimeout(window.__spDefer);
    window.__spDefer = setTimeout(spectateWatch, __pkBusy.sp - Date.now() + 120);
    return;
  }
  openSpectate(g);
}
...
function liveWatch() {
  ...
  if (d !== __liveDigest && Date.now() - __liveAt > 2500) {
    __liveDigest = d; __liveAt = Date.now();
...
document.addEventListener("click", function (e) {
  ...
  if (spb) {
    ...
    if (gg) openSpectate(gg, { replay: gg.phase === "done" });
  }
